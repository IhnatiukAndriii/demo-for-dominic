    ...
    <div className="bg-white/10 backdrop-blur-sm rounded-2xl shadow-2xl border border-white/20 p-8">
      ...
      <div className="space-y-3 mt-6">
        {roles.map((role) => (
          ...
          >
            <div className="flex-shrink-0 p-2.5 rounded-lg bg-white/10 text-brand-200 group-hover:bg-brand-400 group-hover:text-white transition-colors">
              {role.icon}
            </div>
            <div className="flex-1 min-w-0">
              <p className="font-semibold text-white">{role.label}</p>
              <p className="text-xs text-brand-300 mt-0.5">{role.name} · {role.company}</p>
              <p className="text-sm text-brand-200 mt-1">{role.description}</p>
            </div>
            <svg className="w-5 h-5 text-brand-300 group-hover:text-white flex-shrink-0 mt-1 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
